...
public class LoadCfg {
    ...
    public static void loadCfg(){
        if(!new File(Main.getPlugin().getDataFolder(), "config.yml").exists()) {
            ...
            cfg=new Config(new File(Main.getPlugin().getDataFolder(), "config.yml"), Config.YAML);
            Main.getPlugin().getLogger().info(TextFormat.GREEN+"config.yml创建完成!");
        }else {
            cfg = new Config(new File(Main.getPlugin().getDataFolder(), "config.yml"), Config.YAML);
            Main.getPlugin().getLogger().info(TextFormat.YELLOW+"config.yml已找到!");
        }
        reLoadCfg();
