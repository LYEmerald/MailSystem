package net.IchigyouRuri.Mail.mailsN.math;
...
import net.IchigyouRuri.Mail.Utils.RandomId;
...
public class MailMath {
    ...
    public static boolean sendMail(Player sender, Player receiver, Item item,String msg){
        ...
        File pfi=new File(Main.getPlugin().getDataFolder()+File.separator+"Players"+File.separator+"Mail",receiver.getName()+".yml");
        if(!pfi.exists()) {
            return false;
        }
    ...
    public static boolean sendMail(CommandSender sender, String receiver, Item item, String msg){
        String randomId= RandomId.getRandomMailId();
        File pfi=new File(Main.getPlugin().getDataFolder()+File.separator+"Players"+File.separator+"Mail",receiver+".yml");
        if(!pfi.exists()) {
            return false;
        }
        Config pf=new Config(pfi,Config.YAML);
